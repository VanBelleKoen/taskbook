...
class Render {
  ...
  invalidBoardName() {
    const prefix = '\n';
    const message = 'Invalid board name provided';
    error({ prefix, message });
  }

  cannotDeleteDefaultBoard() {
    const prefix = '\n';
    const message = 'Cannot delete the default board without the --force option';
    error({ prefix, message });
  }

  boardNotFound(boardName) {
    const prefix = '\n';
    const message = `Board "${boardName}" not found`;
    error({ prefix, message });
  }

  successDeleteBoard(boardName, stats) {
    const prefix = '\n';
    const message = `Successfully deleted board: ${green(boardName)}`;
    const suffix = grey(`(${stats.totalItemsProcessed} items processed, ${stats.itemsMovedToDefault} moved to default board)`);
    success({ prefix, message, suffix });
  }
}
